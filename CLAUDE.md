...
- **Default `location_type` for `add_zombie_spawner` is `"riser_location"`** (as of v23.6, May 14 2026). Interior spawners now produce zombies that rise from the ground with the canonical zm "claw-up" animation, matching stock `zm_template_test.map` interior risers. The old default of `"spawn_location"` (walk-in / "blink-in standing") looked bad in the user's playtests — zombies would appear standing in the middle of the room with no animation. `riser_location` requires the spawn struct's origin to be on solid navmesh-covered ground (z = floor surface), which `place_zombie_spawners_on_terrain` already ensures via the heightmap sidecar.
- **The MCP-verified zombie spawn patterns are now (1) start_zone barricade+riser+courtyard AND (2) interior `riser_location` with `script_string="find_flesh"`.** Pattern 2 was MCP-unverified through May 13 2026, then runtime-verified in zm_patch_ai_lab_02 (May 14) once the explicit `actor_spawner_zm_factory_zombie` factory was added and the script_string was set. The earlier "interior spawners blink in" finding was a combo of (a) missing factory in some recipes, (b) wrong location_type defaulting to spawn_location, and (c) missing find_flesh script_string. All three are now fixed in the defaults.
...
- **Dog rounds MUST be disabled unless the map has `zombie_dog_spawner` structs.** `zm_usermap::main()` runs `DEFAULT(level.dog_rounds_allowed,1)` then `zm_ai_dogs::enable_dog_rounds()`. At the first hellhound round (round 5 in classic), `_zm_ai_dogs.gsc` spawns from `level.dog_spawners = getEntArray("zombie_dog_spawner","script_noteworthy")`. Procedurally-generated maps have none → empty array → `level.dog_spawners[0]` derefs undefined → fatal SRE `undefined is not an array, string, or vector  scripts/zm/_zm_ai_dogs.gsc:0`. The map plays fine until round 5 then hard-errors. The scaffold GSC template ([scaffold.py](bo3_mcp/scaffold.py) `_gsc_template`) now sets `level.dog_rounds_allowed = 0;` **before** `zm_usermap::main()` (the `DEFAULT` macro is `if(!isdefined)` so a pre-set value wins). To ship hellhounds intentionally, place `zombie_dog_spawner` `script_noteworthy` structs and flip it back to 1. This was latent in EVERY map this MCP produced (incl. zm_demo_v3) — nobody had reached round 5 before May 16 2026.
