...
- **Interior spawn_locations are PARTIALLY WORKING / buggy — even in the original demo v3.** Side-by-side runtime playtest on May 13 2026 of `zm_foundation_v1` and `zm_demo_v3` confirmed: BOTH maps' arena_zone interior spawners produce zombies that "blink in" (not rise from ground) and have glitchy AI tracking — not stuck-in-place, but not smooth either. Verified that arena entity layouts are byte-identical between the two maps, and their GSCs are byte-identical except for the per-map zone_init function name. So this is a BO3 framework limitation with the `script_struct script_noteworthy="spawn_location"` pattern, NOT something the MCP introduced. **Do not treat demo v3's arena/vault zones as "verified working" — only its start_zone pattern is fully verified.**
- **The ONLY fully verified zombie spawn pattern is start_zone's barricade+riser+courtyard.** Zombies rise outside a wall in a courtyard, walk to the barricade window, vault through, then path normally inside the room. The "vault through barricade" step appears to be what registers them with the BO3 AI director. For terrain or open-area maps, use this same pattern (terrain INSIDE the start_zone, or risers OUTSIDE a barricaded perimeter).
